...
IDuelulElementelor::IDuelulElementelor(const std::string &nameOne, const std::string &nameTwo,
    ...
    connect(m_SpecialOptions, &SpecialOptions::OptionSelected, this,
            &IDuelulElementelor::OnModifierSelected);
...
void IDuelulElementelor::OnCardSelected(const int cardIndex) {
...
void IDuelulElementelor::OnModifierSelected(const int modifier) {
    // 1 -> Illusion
...
void IDuelulElementelor::SwitchTurn() {
    ...
    if (m_CurrentGame.ExplosionEnabled() && m_CurrentGame.CheckExplosion()) {
        ...
        connect(explosionDialog, &ExplosionDialog::ExplosionTriggered, this,
                &IDuelulElementelor::OnExplosion);
    ...
    if (currentScore >= m_CurrentGame.GetScoreToWin()) {
        ...
        QTimer::singleShot(1500, this, &IDuelulElementelor::GameFinished);
    } else {
